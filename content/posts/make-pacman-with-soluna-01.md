...
Soluna 引擎的主版本号. 现在没什么用, 后面可以用来校验引擎版本是否符合要求. `width` 和 `height`
分别是游戏窗口的宽度和高度, 我们这里给一个常见的数值.
...
这里我们使用了 `ltask` 库来获取当前的时间. [ltask](https://github.com/cloudwu/ltask) 是 Soluna 引擎底层依赖的一个轻量 Lua 多任务调度库,
